...
var isMaskedIdEthAddress = (ethAddr) => {
  ...
  return ethAddr.length === ETH_ADDRESS_LEN && compare(idMask, ethAddr.slice(0, ACTOR_ID_ETHEREUM_MASK_LEN)) === 0;
};
...
var AddressBls = class _AddressBls extends Address {
   ...
   */
  toBytes = () => u8aConcat([new Uint8Array([this.protocol]), this.payload]);
  /**
  ...
  toString = () => {
    ...
    return this.networkPrefix + this.protocol.toString() + encode(u8aConcat([this.payload, checksum])).toLowerCase();
  };
  /**
...
var AddressDelegated = class _AddressDelegated extends Address {
  ...
  toString = () => {
    const checksum = this.getChecksum();
    return this.networkPrefix + this.protocol.toString() + this.namespace + "f" + encode(u8aConcat([this.subAddress, checksum])).toLowerCase();
  };
  /**
...
var FilEthAddress = class _FilEthAddress extends AddressDelegated {
   ...
   */
  toEthAddressHex = (hexPrefix = true) => `${hexPrefix ? "0x" : ""}${u8aToString(this.subAddress, "hex")}`;
};
export {
  Address,
  AddressActor,
